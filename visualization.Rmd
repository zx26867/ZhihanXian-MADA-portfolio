...
data_all_party %>% 
ggplot(mapping = aes(x = as.Date(modeldate,"%m/%d/%Y"), y = approve_estimate)) + geom_point(aes(color=party),shape=20,size=1,alpha=0.5) + 
scale_colour_manual(values=color_group)+
xlab("") + ylab("") +
scale_x_date(date_labels = "%m/%d",breaks = "1 month") +  
geom_line(mapping = aes(x = as.Date(modeldate,"%m/%d/%Y"), y = approve_estimate, color=party)) + 
...
annotate(geom="text",x=as.Date("2021-10-15"), y=30,label="19.4% Republicans",fontface="bold", color="red") + 
annotate(geom="text",x=as.Date("2021-10-15"), y=90,label="86.2% Democrats",fontface="bold", color="blue") + 
annotate(geom="text",x=as.Date("2021-10-15"), y=50,label="47.6% Independents",fontface="bold", color="brown")
